...
extern int ski_sched_cpu1_preemption_point, ski_sched_cpu2_preemption_point;
extern int ski_init_cpu1_preemption, ski_init_cpu2_preemption;
static target_long last_pc_start;
...
static target_ulong disas_insn(DisasContext *s, target_ulong pc_start)
{
    ...
    */
    ...
    //if (ski_sched_cpu1_do_preemption || ski_sched_cpu2_do_preemption){
    //    printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    ...
    last_pc_start = pc_start;
    /*
    int eip;
    if (ski_init_options_preemption_by_eip || (ski_init_options_preemption_by_access && ski_sched_cpu1_preemption_point==-1 && ski_sched_cpu2_preemption_point == -1)){
        eip = pc_start;
        if (eip == ski_init_cpu1_preemption || eip == ski_init_cpu2_preemption){
            if (s->cc_op != CC_OP_DYNAMIC)
                    gen_op_set_cc_op(s->cc_op);
            gen_helper_ski_instrument_possible_inversion_point(tcg_const_i32(pc_start));
        }
    }
    else if (ski_init_options_preemption_by_access && ski_sched_cpu1_preemption_point && ski_sched_cpu2_preemption_point){
        eip = pc_start;
        if (eip == ski_cpu1_hint_ins || eip == ski_cpu2_hint_ins){
            if (s->cc_op != CC_OP_DYNAMIC)
                    gen_op_set_cc_op(s->cc_op);
            gen_helper_ski_instrument_possible_inversion_point(tcg_const_i32(pc_start));
        }
