...
static inline void dump_memory_access(uint32_t mem_address, uint32_t value, int mem_index, int n_bits, int is_load, FILE *fd)
{
	...
	if(likely(ski_exec_trace_nr_entries < SKI_EXEC_TRACE_MAX_ENTRIES)){
        ...
        if (ski_init_options_short_profile){
            if ((int)env->cpu_index == ski_init_options_set_cpu){
                // Sishuai
                // If the memory access comes from the target process
                if ((env->cpu_index == 0 && env->cr[3] == 0x1237f000) ||
                    (env->cpu_index == 1 && env->cr[3] == 0x122d9000  ))
                {
                    current_stack = ESP & 0XFFFFE000;
                    if (ski_last_eip > kernel_low && ski_last_eip < kernel_high && (mem_address < current_stack || mem_address >= current_stack + 0X2000)){
                        if (is_load == 0){
                            fprintf(ski_mem_write_fd, "### MEM: %d %x %x %x %x %d %c %x\n", (int) env->cpu_index, ski_last_eip, mem_address, value & mask,  mem_index, n_bits, 'S', paddr);
                        }
                        else{
                            fprintf(ski_mem_read_fd, "### MEM: %d %x %x %x %x %d %c %x\n", (int) env->cpu_index, ski_last_eip, mem_address, value & mask,  mem_index, n_bits, 'L', paddr);
                        }
                    }
                }
            }
        ...
        if(ski_forkall_enabled){
            if (ski_init_options_preemption_by_eip){
                if (ski_last_eip > 0xc0000000 && ski_last_eip != ski_init_cpu1_preemption && ski_last_eip != ski_init_cpu2_preemption)
					ski_stats_add_data_access(mem_address, ski_last_eip, n_bits/8, is_load?0:1, env->cpu_index);
                    //ski_stats_add_data_access(mem_address, ski_last_eip, n_bits/8, is_load?0:1, env->cpu_index, value & mask);
            }
            /*
            current_stack = ESP & 0XFFFFE000;
            if (ski_last_eip > kernel_low && ski_last_eip < kernel_high && (mem_address < current_stack || mem_address >= current_stack + 0X2000)){   
                // preemption by eip mode should be removed
                // ski_stats is only useful when we evaluate SKI
                
                if (!ski_init_options_preemption_by_eip && !ski_init_options_preemption_by_access)
                {   
                    ski_stats_add_data_access(mem_address, ski_last_eip, n_bits/8, is_load?0:1, env->cpu_index);
                }
                else{
                    if (ski_init_options_preemption_by_eip){
                        if ((env->cpu_index==0 && ski_last_eip == ski_init_cpu1_preemption) || (env->cpu_index==1 && ski_last_eip==ski_init_cpu2_preemption)){
                            ski_stats_add_data_access(mem_address, ski_last_eip, n_bits/8, is_load?0:1, env->cpu_index);
                        }
                    }
                    else{
                        if (mem_address == ski_init_preemption_channel_addr){
                            //printf("============================== SKI debug ==============================\n");
                            //printf("SKI_HELPER memory address is saw, accessed by IP %08x\n", ski_last_eip);
                            //printf("============================== SKI debug ==============================\n");
                            ski_stats_add_data_access(mem_address, ski_last_eip, n_bits/8, is_load?0:1, env->cpu_index);
                        }
                    }
	...
	if(env->ski_active == false){
		...
		if(unlikely(ski_correct_thread()==false)){
            ...
            */
			...
			if(unlikely((!ski_init_options_preemption_by_eip) && (!ski_init_options_preemption_by_access) && (ski_sched_instructions_total == env->ski_cpu.nr_max_instr))){
                SKI_TRACE("helper_ski_instrument_mov: reached %d instructions\n", env->ski_cpu.nr_instr_executed);
				SKI_TRACE("ski_sched_instructions_total = %d, ski_sched_instructions_current = %d\n", ski_sched_instructions_total, ski_sched_instructions_current);
				do_preemption = 1;
			}
            // Sishuai
            // Add a preemption point if the preemption flag is set according to the memory access history
            if (ski_init_options_preemption_by_access){
                int random_preempt = -1;
                if (ski_sched_cpu1_do_preemption){
                    random_preempt = random() % 2;
                    if (random_preempt == 1){
                        do_preemption = 1;
                    }
                    ski_sched_cpu1_do_preemption = 0;
                }
                if (ski_sched_cpu2_do_preemption){
                    random_preempt = random() % 2;
                    if (random_preempt == 1){
                        do_preemption = 1;
                    }
                    ski_sched_cpu2_do_preemption = 0;
                }
            }

            // Sishuai
            // Also add a preemption point if the current instruction is a target instructions
            if(ski_init_options_preemption_by_access && !do_preemption)
            {
                //ski_preeemption_by_eip_entry* head = ski_preeemption_by_eip;
				//ski_preeemption_by_eip_entry* el;
                int random_seed = -1;
                int communication_ins = 0;
                if (env->cpu_index == 0){
                    if ((snowboard_cpu1_preempt_count < 3 || ski_cpu1_preempt_count < 6) && cpu0_nr_instr_executed +1 != env->ski_cpu.nr_instr_executed){
                        if (snowboard_cpu1_preempt_count < 3 && ski_init_cpu1_preemption == explicit_eip){
                            communication_ins = 2;
                        }
                        if (ski_cpu1_preempt_count < 6 && !communication_ins && ski_init_options_preemption_by_eip){
                            ski_ipfilter_hash_entry *e;
                            if (ski_ipfilter_hash){
                                HASH_FIND_INT(ski_ipfilter_hash, &explicit_eip, e);
                                if (e){
                                    communication_ins = 1;
                                    HASH_DEL(ski_ipfilter_hash, e);
                                }
                            }
                        }
                        if (communication_ins){
                            if (communication_ins == 1){
                                random_seed = random() % 30;
                            }
                            else{
                                random_seed = random() % 2;
                            }
                            // if cpu1 is not stopped before and should add preemption by random choice
                            if (!ski_cpu1_preempt && random_seed == 1){
                                if (communication_ins == 1){
                                    ski_cpu1_preempt_count++;
                                }
                                else{
                                    snowboard_cpu1_preempt_count++;
                                }
                                do_preemption = 1;
                                cpu0_nr_instr_executed = env->ski_cpu.nr_instr_executed;
                            }
                            // This is to avoid the situation where one CPU is preempted in a dead loop
                            if (ski_cpu1_preempt){
                                ski_cpu1_preempt = 0;
                            }
                        }
                    ...
                    if ((snowboard_cpu2_preempt_count < 3 || ski_cpu2_preempt_count < 6) && cpu1_nr_instr_executed +1 != env->ski_cpu.nr_instr_executed){
                        if (snowboard_cpu2_preempt_count < 3 && ski_init_cpu2_preemption == explicit_eip){
                            communication_ins = 2;
                        }
                        if (ski_cpu2_preempt_count < 6 && !communication_ins && ski_init_options_preemption_by_eip){
                            ski_ipfilter_hash_entry *e;
                            if (ski_ipfilter_hash){
                                HASH_FIND_INT(ski_ipfilter_hash, &explicit_eip, e);
                                if (e){
                        ...
                        if (communication_ins){
                            ...
                            if (!ski_cpu2_preempt && random_seed == 1){
                                if (communication_ins == 1){
                                    ski_cpu2_preempt_count++;
                                }
                                else{
                                    snowboard_cpu2_preempt_count++;
                                }
                                do_preemption = 1;
                                cpu1_nr_instr_executed = env->ski_cpu.nr_instr_executed;
                            }
                            if (ski_cpu2_preempt){
                                ski_cpu2_preempt = 0;
                            }
...
void helper_ski_instrument_all(int explicit_eip)
{
	...
	if(likely(env->ski_active == false)){
		...
		if(ski_exec_trace_execution_fd && (ski_exec_trace_nr_entries < SKI_EXEC_TRACE_MAX_ENTRIES)){
			...
			if(ski_forkall_enabled){
                if (explicit_eip > kernel_low && explicit_eip < kernel_high){
                    if (!ski_init_options_preemption_by_access){
