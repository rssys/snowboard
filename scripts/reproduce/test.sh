...
error_msg(){
        echo [REPRODUCE.SH] ERROR: $1
        echo [REPRODUCE.SH] ERROR: exiting!
        exit 1
}
...
# ask for output path if not provided before running
...
SKI_OUTPUT_DIR="$SKI_OUTPUT_DIR/reproduce-`date +'%Y-%m-%d-%H-%M-%S'`"
if ! [ -d "$SKI_OUTPUT_DIR" ]
then
...
VMM_MISC_OPTIONS="-rtc base=utc,clock=vm -qmp tcp:localhost:10000,server,nowait -net nic -net user,hostfwd=tcp::10001-:22 -vnc :1,password -monitor telnet:127.0.0.1:1235,server,nowait"
VMM_SKI_ARGS="-ski 0,input_number=210:210,preemptions=119:97"

# some verification
if ! [ -x $VMM_BINARY ] ; then error_msg "Unable to find the binary at $VMM_BINARY."; fi
...
export SKI_PREEMPTION_BY_ACCESS=1
...
export SKI_WATCHDOG_SECONDS=20
...
#SKI_KERNEL_FILENAME=$SKI_DIR/test/target_kernel/5.3.11/5.3.11_bzImage
...
# where is my kernel console output???
